...
app.get('/', (req, res) => {
    const documentJob = identityDocumentQueue.createJob({ userId: "1342132154132", documentKey: "passport1.jpg" })
    documentJob.save();
    res.send('Job added!')
}
)
...
identityDocumentQueue.process(function (job, done) {
    ...
    var filePath = job.data.documentKey;
    
    var stdout = require('child_process').execSync(`mrz --json ${filePath}`);
    console.log(`[CV] Processing Task Completed.`)
    console.log(stdout);
    var matchedPassport = JSON.parse(stdout)
    if  (matchedPassport.valid_score > 50) {
        ...
        return done(null, STATUS_SUCCESS);
    } else {
        console.log("[CV] No valid document detected.")
        return done(null, STATUS_INVALID);
    }
