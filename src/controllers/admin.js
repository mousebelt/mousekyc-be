...
exports.postApproveUser = async (req, res) => {
  ...
  let { approvalDescription } = req.body;
...
exports.getUserDocuments = async (req, res) => {
  ...
  try {
    ...
    for(let i = 0; i < docTypes.length; i += 1) {
      ...
      if(!filename || filename === '') continue; // eslint-disable-line

      try {
        const file = await UtilsModule.getImageDataFromGrid(gfs, filename); // eslint-disable-line
        if(file) {
          const item = {};
          item[key] = file;
          documents.push(item);
...
exports.getVerifyOwner = async (req, res) => {
  const { ownerConfirmToken } = req.params;
